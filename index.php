...
?>
    ...
    <table cellpadding="5">
        <tr>
            ...
            <?php
            $sql = "SELECT autori.id, titlu, nume_autor, pret, carti.id FROM carti, autori WHERE carti.id_autor=autori.id ORDER BY data DESC LIMIT 0,3";
            $resursa = mysqli_query($db, $sql);
            while($row = mysqli_fetch_array($resursa))
            {
                /*deschidem celula tabelului html*/
                print '<td align="center">';
                $adresaImagine = "coperte".$row['id'].".jpg";
                ...
                print '<b><a href="carte.php?id_carte='.$row['id'].'">'.$row['titlu'].'</a></b><br> de <i>'.$row['nume_autor'].'</i><br> Pret: '.$row['pret'].'lei</td>';
            }
            ?>
        </tr>
    </table>
    ...
    <table cellpadding="5">
        <tr>
<?php
$sqlVanzari = "SELECT carti.id, sum(nr_buc) AS bucatiVandute FROM vanzari, carti GROUP BY carti.id ORDER BY bucatiVandute DESC LIMIT 0,3";
...
while($rowVanzari = mysqli_fetch_array($resursaVanzari))
{
    ...
    {
        print '<td align = "center">';
        $adresaImagine = "coperte/".$rowVanzari['id'].".jpg";
        if(file_exists($adresaImagine))
        {
            print '<img src = "'.$adresaImagine.'" width="75" height="100"><br>';
        }
